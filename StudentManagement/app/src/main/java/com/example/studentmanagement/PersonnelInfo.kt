...
class PersonnelInfo : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //PersonnelList에서 putExtra로 넘겨준 str_name(intent_name)을 받음
        val intent = intent
        ...
        var cursor: Cursor
        cursor = sqllitedb.rawQuery("SELECT * FROM personnel WHERE name = '$str_name';", null)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item?.itemId) {
            ...
            R.id.action_reg -> {
                val intent = Intent(this, PersonnelRegist::class.java)
                startActivity(intent)
                return true
            }
            R.id.action_remove -> {
                dbManager = DBManager(this, "personnel", null, 1)
                sqllitedb = dbManager.readableDatabase
                sqllitedb.execSQL("DELETE FROM personnel WHERE name = '$str_name';")
                sqllitedb.close()
                dbManager.close()
